...
function submit(currentTabID) {
    if (newCookie)
        ...
        submitAll(currentTabID);    /// This CASE
}
/// +>>>> Hàm chính xữ lý Import Cookie
function submitAll(currentTabID) {
    ...
    cookies.each(function () {
        var cCookie = formCookieData($(this));  // Tạo lại cookie với tham số đã thay đổi

        if (cCookie === undefined) {
            return;
        }

        deleteCookie(cCookie.url, cCookie.name, cCookie.storeId, function () {  /// Xoá cookie củ và import lại cookie mới vừa chỉnh sửa
